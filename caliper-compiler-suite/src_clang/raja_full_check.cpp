...
public:
  ...
  void verifyCapture(const clang::LambdaCapture capture)
  {
    //llvm::outs() << "SO I FOR ONE THINK PUPPIES ARE ADORABLE\n";
    assignmentType lastAssignment(NULL, NULL);
    // get the location that caused the variable to be captured (its use
    // location in the kernel)
    SourceRange useRange = capture.getLocation();
    // across statements that came before the kernel
    for (const Stmt *prologueStatement : getPrologue()) {
      ...
      if (hasValidLHS(candidateAssignment)
        ...
        // update it to be the last assignment to that variable
        lastAssignment = candidateAssignment;
      }
    }
    if (hasValidLHS(lastAssignment)) {
      // get the right hand side as a function call
      operateOnType<CallExpr>(
          getRHS(lastAssignment), [&](const CallExpr *assignedToExpr) {
      ...
      operateOnType<ConditionalOperator>(
          getRHS(lastAssignment), [&](const ConditionalOperator *conditional) {
  ...
  template <int CountCategory>
  void dumpLocation(Stmt *stm, int times = 1)
  {
    static int done;
    if (done++ < times) {
      llvm::errs() << CountCategory << ": "
                   << stm->getLocStart().printToString(
                          TheASTContext.getSourceManager())
                   << "\n";
    }
  }
  ...
  bool VisitRajaKernel(const RajaKernel *kernel)
  {
    ...
    for (const clang::LambdaCapture capture : getCaptures()) {
...
public:
      ...
      operateOnType<VarDecl>(deref->getDecl(), [&](const VarDecl* refdVar){
        std::vector<const Stmt*> previousExprs = scanFuncBefore<Stmt>(TheASTContext, getEnclosingFunction(), deref, [=](const Stmt* prologueStatement){
          ...
          if (hasValidLHS(candidateAssignment)
              && nameMatches(getLHS(candidateAssignment),
                             refdVar)) {
            if(inRaja(getRHS(candidateAssignment))){
              return false;
            }
            operateOnType<CallExpr>(
                getRHS(candidateAssignment), [&](const CallExpr *assignedToExpr) {
                  ...
                  // the regex we were told to look for then we have a problem
                  if (std::regex_search(getNameOfCalled(assignedToExpr), *m_regex)) {
                    warnOutsideUse(assignedToExpr->getExprLoc(), useRange);
                  }
                });
                // get the right hand side as a conditional expression
                // a conditional expression has the form "foo = condition ? something :
  ...
  bool VisitRajaKernel(RajaKernel* in){ return true; }
}; //class UseOutsideRajaVisitor
}  // end anonymous namespace

static FrontendPluginRegistry::Add<RegexVisitorScaffolding<SampleRajaVisitor>::AddTemplateType> X("lambda-checker",
                                                          "print function "
                                                          "names");
//static FrontendPluginRegistry::Add<RegexVisitorScaffolding<UseOutsideRajaVisitor>::AddTemplateType> Y("use-checker",
//                                                          "print function "
//                                                          "names");
